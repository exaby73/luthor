...
<div className="feature-summary">
  <p><strong>Type-safe SchemaKeys</strong> for schema definition</p>
  <p><strong>Type-safe ErrorKeys</strong> for error handling</p>
  <p><strong>Cross-field validation</strong> with schema custom validators</p>
  <p><strong>JsonKey integration</strong> for API compatibility</p>
  <p><strong>Nested schema validation</strong></p>
</div>
...
bool endDateAfterStartDate(Object? value, Map<String, Object?> data) {
  if (value is String && data['start_date'] is String) {
...
bool acceptedTerms(Object? value) {
  return value == true;
}
...
abstract class Employment with _$Employment {
  const factory Employment({
    ...
    @HasMinDouble(0) @HasMaxDouble(200000) required double salary,
  }) = _Employment;
...
abstract class UserRegistration with _$UserRegistration {
  const factory UserRegistration({
    ...
    @HasMin(8) @MatchRegex(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)')
    ...
    // Terms acceptance
    @WithCustomValidator(acceptedTerms, message: 'Terms must be accepted')
...
void main() {
  ...
  print('💼 Testing employment date validation...');
...
void testInvalidRegistration() {
  ...
  switch (result) {
    ...
    case SchemaValidationError():
      print('✅ Validation failed as expected. Errors found:');
      ...
      if (firstNameError != null) print('   • First Name: $firstNameError');
      if (lastNameError != null) print('   • Last Name: $lastNameError');
      if (emailError != null) print('   • Email: $emailError');
      if (passwordError != null) print('   • Password: $passwordError');
      if (confirmPasswordError != null) print('   • Confirm Password: $confirmPasswordError');
      if (phoneError != null) print('   • Phone: $phoneError');
      if (ageError != null) print('   • Age: $ageError');
      if (cityError != null) print('   • City: $cityError');
      if (termsError != null) print('   • Terms: $termsError');
  }
}
...
void testBusinessAccountValidation() {
  ...
  switch (result1) {
    case SchemaValidationError():
      ...
      print('✅ Business account validation: $phoneError');
  ...
  switch (result2) {
    case SchemaValidationSuccess():
      print('✅ Personal account doesn\'t require phone number');
    case SchemaValidationError():
...
void testEmploymentValidation() {
  ...
  switch (result) {
    case SchemaValidationError():
      ...
      print('✅ Employment date validation: $endDateError');
    case SchemaValidationSuccess():
...
✅ Validation failed as expected. Errors found:
   • First Name: firstName must be at least 2 characters long
   ...
   • Phone: Phone number is required for business accounts
   • Age: age must be at least 18
   • City: city must be at least 1 characters long
   • Terms: acceptTerms is required

==================================================

🏢 Testing business account validation...
✅ Business account validation: Phone number is required for business accounts
✅ Personal account doesn't require phone number

==================================================

💼 Testing employment date validation...
✅ Employment date validation: End date must be after start date
```
