...
abstract class Comment with _$Comment {
  const factory Comment({
    required String id,
    required String text,
    List<Comment>? replies,  // Auto-detected
    Comment? parent,        // Auto-detected
    @luthorForwardRef User? user,  // Explicit annotation needed
  }) = _Comment;
...
- Applies `forwardRef()` when the value type matches the enclosing class or is auto-detected
- Handles nullable types correctly (omits `.required()` when appropriate)
